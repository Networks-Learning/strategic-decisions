...
do
  ...
  do
    ...
    for k in {0..2}
    do
      python -m lib.iterative --output=./outputs/additive_iterative_kappa_${kappa[$k]}_m_${m[$j]}_seed_${seed[$i]} --m=${m[$j]} --njobs=1 --kappa=${kappa[$k]} --seed=${seed[$i]} --additive --gamma=$gamma
